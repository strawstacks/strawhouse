...
var Cmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		var mode enum.SignatureMode
		if modeFlag == "file" {
			mode = enum.SignatureModeFile
		} else if modeFlag == "dir" {
			mode = enum.SignatureModeDirectory
		} else {
		...
		var action enum.SignatureAction
		if actionFlag == "get" {
			action = enum.SignatureActionGet
		} else if actionFlag == "upload" {
			action = enum.SignatureActionUpload
		} else {
