...
func TestVerify(t *testing.T) {
	...
	token := signature.Generate(SignatureActionGet, SignatureModeFile, "/photo/2023/1217-ctouting1/1.jpg", false, time.Now().Add(2*time.Second), attr)

	attribute, err := signature.VerifyInt(SignatureActionGet, "/photo/2023/1217-ctouting1/1.jpg", token)
	assert.Equal(t, err, nil)
	assert.Equal(t, attribute, attr)
	...
	token = signature.Generate(SignatureActionUpload, SignatureModeDirectory, "/photo/2024/0628-sao-kmutt-seminar/", false, time.Now().Add(20*time.Minute), attr)
	...
	token = signature.Generate(SignatureActionGet, SignatureModeDirectory, "/photo/2024/1127-sit-dday/", true, time.Now().Add(20*time.Minute), attr)
	...
	token = signature.Generate(SignatureActionUpload, SignatureModeDirectory, "/photo/2023/1217-ctouting1/", false, time.Now().Add(-2*time.Second), attr)
...
func BenchmarkVerify(b *testing.B) {
	signature, attr := Prepare()
	token := signature.Generate(SignatureActionUpload, SignatureModeDirectory, "/photo/2024/", true, time.Now().Add(20*time.Minute), attr)
	for n := 0; n < b.N; n++ {
